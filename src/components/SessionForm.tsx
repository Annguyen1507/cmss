...
import { getArticleById } from "../features/article/api/article.service";
...
export default function SessionForm({ sessionId, onClose, onSubmit }: SessionFormProps) {
    ...
    useEffect(() => {
        ...
        async function fetchArticle() {
            setLoading(true);
            try {
                const response = await getArticleById(sessionId!);
            ...
            >
                <div className="flex h-[68px] items-center justify-between border-b border-[#D8D8D8] px-6">
                    <h2 className="text-[22px] font-medium text-[#111]">
                        {sessionId ? "Edit PD session" : "Create PD session"}
                    </h2>
                        ...
                        <div className="space-y-4">
                            {Array.from({ length: 7 }).map((_, i) => (
                                <div key={i}>
                                    ...
                                    <div className="h-[48px] w-full animate-pulse rounded-md bg-gray-100" />
                                </div>
                            ))}
                        </div>
                    ) : (
                            ...
                            <div>
                                ...
                                <Controller
                                    ...
                                    render={({ field: { onChange, value, ref } }) => (
                                        <div ref={ref} tabIndex={-1} className="inline-flex overflow-hidden rounded-md border border-[#93C5FD]">
                                            {(["published", "unpublished", "draft"] as const).map((option, index) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`cursor-pointer h-[44px] px-6 text-[14px] font-semibold uppercase tracking-wide transition ${index !== 0 ? "border-l border-[#93C5FD]" : ""
                                                            } ${isSelected
                                ...
                                {errors.status && (
                                    <p className="mt-1 text-sm text-[#E53935]">{errors.status.message}</p>
                                )}
                            </div>
